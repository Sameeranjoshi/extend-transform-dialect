...
// GetChildForOp
//===----------------------------------------------------------------------===//
// This apply function get's automatically generated from the tablegen defn of the Op.
// Let's write the main logic into it.

DiagnosedSilenceableFailure
transform::GetChildForOp::apply(transform::TransformResults &results,
                                 transform::TransformState &state) {
  SetVector<Operation *> childs;
  for (Operation *target : state.getPayloadOps(getTarget())) { // assuming multiple targets which the op might point to, who knows be safe!
    target->getName().print(llvm::outs());
    scf::ForOp loop;
    Operation *current = target;
//    for (unsigned i = 0, e = getNumLoops(); i < e; ++i) {
      // Loop over the parameter the user has given in the attributes num_loops= 3
      // current->getName().print(llvm::outs());
      loop = current->getChildOfType<scf::ForOp>();
      if (!loop) {
      //  This is an error where a loop is not found.
        DiagnosedSilenceableFailure diag = emitSilenceableError()
                                           << "could not find an '"
                                           << scf::ForOp::getOperationName()
                                           << "' child";
        diag.attachNote(target->getLoc()) << "target op";
      // Look result variable is kept blank
        results.set(getResult().cast<OpResult>(), {});
        return diag;
      }
      current = loop;
//    }
  ...
  results.set(getResult().cast<OpResult>(), childs.getArrayRef());
  return DiagnosedSilenceableFailure::success();
}
